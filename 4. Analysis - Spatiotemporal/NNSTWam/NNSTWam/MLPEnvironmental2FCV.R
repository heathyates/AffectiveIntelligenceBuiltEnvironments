...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- nnet(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "nn_environmental_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "NNEnvironmental2FCV")
