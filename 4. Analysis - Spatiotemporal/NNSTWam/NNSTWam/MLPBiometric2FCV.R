...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- nnet(BINARYANNOTATION ~ HRZONENORM + EDAZONENORM,
               size = 4,
               decay = 0.0001,
               maxit = 1000,
               data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "nn_biometric_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "NNBiometric2FCV")
