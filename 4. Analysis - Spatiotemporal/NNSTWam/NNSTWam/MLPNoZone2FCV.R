...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- nnet(BINARYANNOTATION ~ HRZONENORM + EDAZONENORM + WALKABILITY + NUMLIGHTS +NUMTREES
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "nn_nozone_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "NNNoZone2FCV")
