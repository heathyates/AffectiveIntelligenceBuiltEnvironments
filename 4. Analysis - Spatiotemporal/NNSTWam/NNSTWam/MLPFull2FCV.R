...
CalculateModelPerformanceMetrics <- function(fold, fit, testData) {
    ...
    fitted.results <- predict(fit, testData, type = "raw")
...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- nnet(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES + NUMINFLINE + NUMINFPOIN
                                                          + NUMGRASS + NUMSCRUB + as.factor(ZONE) + as.factor(GENDER) + as.factor(BODYSHAPE) + as.factor(URBANORIGIN)
                                                          + as.factor(URBANPREFERENCE) + as.factor(STUDYAREAFAMILIARITY) + as.factor(EXERCISE) + HRZONENORM
                                                          + EDAZONENORM + PARTICIPANTID,
                  size = 4,
                  decay = 0.0001,
                  maxit = 1000,
                  data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "nn_full_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "NNFull2FCV")
