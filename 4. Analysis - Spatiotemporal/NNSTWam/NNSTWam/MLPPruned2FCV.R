...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        #fit <- glm(as.factor(BINARYANNOTATION) ~ ZONE + WALKABILITY + NUMLIGHTS + NUMTREES
               ...
               #data = trainData)
        fit <- nnet(BINARYANNOTATION ~ as.factor(ZONE) + HRZONENORM + EDAZONENORM + PARTICIPANTID,
                    size = 4,
                    decay = 0.0001,
                    maxit = 1000,
                    data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "nn_pruned_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "NNPruned2FCV")
