...
CalculateModelPerformanceMetrics <- function(fold, fit, testData) {
    ...
    missclassificationError <- mean(fit != testData$BINARYANNOTATION)
    ...
    #Step 2.1: Create the prediction object and levels 
    predictions <- rep(2, length(fit)) #Change to 2 for positive 
    ...
    #Step 4: Calculate the number of negatives in the data set 
    percentageNegativeInTestData <- sum(testData$BINARYANNOTATION == 0) / nrow(testData)
    percentagePostiveInTestData <- 1 - percentageNegativeInTestData

    #Step 3: Create a row and return the values 
    row <- cbind(FOLD = fold, ACCURACY = accuracy, AUC = auc, PERCENTNEG = percentageNegativeInTestData, PERCENTPOS = percentagePostiveInTestData)
...
RunRandomForest <- function(dataParentDirectory, exportDirectoryLoocv) {
    #Random Forest 
    #
    #Args: 
    #   dataParentDirectory: File path to the parent directory of the data for training and testing  
    ...
    print("Beginning script for running naive positive models")
    set.seed(415)
    ...
    modelPerformanceColumns <- c("FOLD", "ACCURACY", "AUC", "PERCENTNEG", "PERCENTPOS")
    ...
    for (i in 1:length(trainFiles)) {
        ...
        print(paste0("Naive positive fitting for fold: ", i))
        sizeOfTestData <- nrow(testData)
        fit <- rep(1, sizeOfTestData)

        #Step 2.5: Calculate the model metrics 
        print(paste0("Conducting model performance for fold: ", i))
        rowOutput <- CalculateModelPerformanceMetrics(i, fit, testData)


        #Step 2.6: Add this to the output
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "naivepositives_model_2fcv.csv")
...
RandomForest <- function() {
    ...
    #Returns:
    #   None (but does write/output training and testing data sets) 
    print("Starting random forest loocv script.")

    #Step 0: Define the input parent directory with the data that contains training and testing as subdirectories 
    dataParentDirectory <- "C:/Users/heath/OneDrive/Documents/Research/Current/Dissertation/3. WAM/12. Appendix - ST Training and Test Data/TWOFCV"
    exportDirectory <- "C:/Users/heath/OneDrive/Documents/Research/Current/Dissertation/3. WAM/5. Results - Spatiotemporal"
    exportDirectoryLoocv <- file.path(exportDirectory, "NAIVEAllPositive2FCV.R")

    #Step 1: Create directory for random forest loocv (remove if there) 
    RemoveAndCreateDirectories(exportDirectoryLoocv)

    #Step 1: Define the method to run the parent directory 
    RunRandomForest(dataParentDirectory, exportDirectoryLoocv)

    print("Finished random all positives model script")
