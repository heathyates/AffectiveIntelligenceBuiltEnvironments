...
CalculateModelPerformanceMetrics <- function(fold, fit, testData) {
    ...
    predictions <- rep(1, length(fit)) #Change to 2 for positive 
...
RunRandomForest <- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        print(paste0("Naive negative fitting for fold: ", i))
        sizeOfTestData <- nrow(testData)
        fit <- rep(0, sizeOfTestData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "naivenegatives_model_2fcv.csv")
...
RandomForest <- function() {
    ...
    exportDirectoryLoocv <- file.path(exportDirectory, "NAIVEAllNegatives2FCV")
    ...
    print("Finished random forest loocv script")
