...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- svm(BINARYANNOTATION ~ HRZONENORM + EDAZONENORM + WALKABILITY + NUMLIGHTS +NUMTREES
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "svm_nozone_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "SVMNoZone2FCV")
