...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- svm(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "svm_environmental_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "SVMEnvironmental2FCV")
