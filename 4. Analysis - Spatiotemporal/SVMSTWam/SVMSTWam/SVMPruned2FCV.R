...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        #fit <- glm(as.factor(BINARYANNOTATION) ~ ZONE + WALKABILITY + NUMLIGHTS + NUMTREES
               ...
               #data = trainData)
        fit <- svm(BINARYANNOTATION ~ as.factor(ZONE) + HRZONENORM + EDAZONENORM + PARTICIPANTID,
                   cost = 100,
                   gamma = 1, 
                   data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "svm_pruned_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "SVMPruned2FCV")
