...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- svm(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES + NUMINFLINE + NUMINFPOIN
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "svm_full_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "SVMFull2FCV")
