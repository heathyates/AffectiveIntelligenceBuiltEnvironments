...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- svm(BINARYANNOTATION ~ HRZONENORM + EDAZONENORM,
               cost = 100,
               gamma = 1, 
               data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "svm_biometric_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "SVMBiometric2FCV")
