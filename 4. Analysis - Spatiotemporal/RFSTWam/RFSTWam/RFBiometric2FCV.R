...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- randomForest(BINARYANNOTATION ~ HRZONENORM + EDAZONENORM,
               data = trainData,
               importance = TRUE,
               ntree = 500)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "rf_biometric_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "RFBiometric2FCV")
