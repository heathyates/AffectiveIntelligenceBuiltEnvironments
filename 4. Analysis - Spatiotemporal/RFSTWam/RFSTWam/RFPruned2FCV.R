...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        #fit <- glm(as.factor(BINARYANNOTATION) ~ ZONE + WALKABILITY + NUMLIGHTS + NUMTREES
               ...
               #data = trainData)
        fit <- randomForest(BINARYANNOTATION ~ ZONE + HRZONENORM  + EDAZONENORM + PARTICIPANTID,
                                               data = trainData,
                                               importance = TRUE,
                                               ntree = 500)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "rf_pruned_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "RFPruned2FCV")
