...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- randomForest(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "rf_environmental_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "RFEnvironmental2FCV")
