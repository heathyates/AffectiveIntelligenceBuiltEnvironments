...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- randomForest(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES + NUMINFLINE + NUMINFPOIN
                                                          + NUMGRASS + NUMSCRUB + ZONE + GENDER + BODYSHAPE + URBANORIGIN
                                                          + URBANPREFERENCE + STUDYAREAFAMILIARITY + EXERCISE + HRZONENORM
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "rf_full_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "RFFull2FCV")
