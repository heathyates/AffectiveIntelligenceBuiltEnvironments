...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- glm(BINARYANNOTATION ~ WALKABILITY + NUMLIGHTS + NUMTREES + NUMINFLINE + NUMINFPOIN
                                                          + NUMGRASS + NUMSCRUB + factor(ZONE) + factor(GENDER) + factor(BODYSHAPE) + factor(URBANORIGIN)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "log_full_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "LOGFull2FCV")
