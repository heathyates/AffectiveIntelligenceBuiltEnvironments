...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        fit <- glm(BINARYANNOTATION ~ HRZONENORM + EDAZONENORM,
               family = binomial,
               data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "log_biometric_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "LOGBiometric2FCV")
