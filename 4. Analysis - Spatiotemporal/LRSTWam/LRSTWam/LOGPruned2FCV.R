...
RunAlgorithm<- function(dataParentDirectory, exportDirectoryLoocv) {
    ...
    for (i in 1:length(trainFiles)) {
        ...
        #fit <- glm(as.factor(BINARYANNOTATION) ~ ZONE + WALKABILITY + NUMLIGHTS + NUMTREES
               ...
               #data = trainData)
        fit <- glm(BINARYANNOTATION ~ factor(ZONE) + HRZONENORM  + EDAZONENORM + PARTICIPANTID,
                                             family = binomial(link = 'logit'),
                                             data = trainData)
    ...
    modelPerformanceFilePath <- file.path(exportDirectoryLoocv, "log_pruned_2fcv.csv")
...
Main <- function() {
    ...
    exportDirectoryForModel <- file.path(exportDirectory, "LOGPruned2FCV")
